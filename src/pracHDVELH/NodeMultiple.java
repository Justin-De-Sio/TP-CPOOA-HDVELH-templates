...
public class NodeMultiple {
    ...
    boolean isFinal() {
        ...
        if (!hasDaughters())
            return true;
        int i = 0;
        while (i < getDaughters().length && getDaughter()[i]==null) {
            i++;

        }
        if(i>=getDaughters().length)
            return true;
        return false;
    }
    ...
    /**
     * @return {@code true} if and only if thi        return getDaughters();
s node has at least one non-null
     * daughter node.
     */
